...
for g in files:
	...
	for i in range(0, 40476):	  	        #Number of atoms
		k = type[i]
		for j in range(0, 238):   	        #Number of ions 
		        sodx = list1[j]
		        sody = list2[j]
			sodz = list3[j]
			if k == 11:	  	        #Type of surrounding the ion		
				deltax = abs(x[i] - sodx)
		                if deltax >= 0.5:
			                deltax -= 0.5
				deltax *= 173.331
		...
		if len(dx) > 1:
			for q in range(0, 238):		#Number of ions
				xval = dx[q]
				yval = dy[q]
				zval = dz[q]
				dis = math.sqrt(xval**2 + yval**2 + zval**2)		
				dislist.append(dis)
				if len(dislist) == 238: #Number of ions
	...
	for i in range(0, 40476):			#Number of atoms
		...
		for l in range(0, 200):			#Number of sds/dtab
			...
			if len(suldz) == 200:		#Number of sds/dtab
				...
				suldy = []		
				suldz = []													 
	for r in range(0, 238): 			#Number of ions
		smin = minsuldislist[r]
		listval = [item[r] for item in biglist]
		for s in range(0, 38):		#Number of surrounding of ion
			disval = listval[s]
			if disval <= 3.75:		#Mindis (see excel table)
				nearwater += 1
